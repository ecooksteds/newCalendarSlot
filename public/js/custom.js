...
function checkBestSlot(e) {
    ...
    if (optionalSlot.length > 0) {
        axios.post("/checkslot", optionalSlot)
            .then((data) => {
...
function displaySearchData(data) {
    ...
    if (data.length > 0) {
        ...
        data.forEach(function (record) {
            ...
            btn.onclick = (event) => { event.preventDefault(); acceptSlotFN(record) }
...
function acceptSlotFN(data) {
    ...
    axios.post("/acceptmeeting", data)
        .then((data) => {
            alert(data.data.message)
            optionalSlot = [];
            window.location.href = "/home"
        })
        .catch((err) => { alert(err); optionalSlot = []; })
}