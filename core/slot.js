...
Slot.prototype = {
    ...
    check: function (res, body, callback) {
        pool.query(`select * from slot where meetingID='${body[0].meetingID}'`, function (err, result) {
            if (err) {
                ...
                for (let i = 0; i < body.length; i++) {
                    ...
                    for (let index = 0; index < result.length; index++) {
                        ...
                        if (record.date.toDateString() == date.toDateString() && record.day == userSlot.day && record.timeStart == userSlot.timeStart && record.timeEnd == userSlot.timeEnd) {
