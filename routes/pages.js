...
// Get home page
router.get('/home', require('connect-ensure-login').ensureLoggedIn(), (req, res) => {
    let user = req.user
    ...
    if (user) {
        ...
        join.create(userInput, function (lastId) {
            if (lastId) {
                ...
                res.send(`<script>alert("Your Meeting Data Has Been Saved"); window.location.pathname="/setmeeting/${lastId.toString()}"</script>`)
            }
            else {
...
// Post login data
router.post('/login', passport.authenticate('local', { successReturnToOrRedirect: '/home', failureRedirect: '/login' }), (req, res) => {
...
router.post('/register', (req, res) => {
    ...
    user.create(userInput, function (lastId) {
        ...
        if (lastId) {
            ...
            //     res.redirect('/home');
            // });
            res.send(`<script>alert("Your Account Has Been Created. Please Login Now!");window.location.pathname="/login"</script>`)
        } else {
...
router.get('/joinmeeting', (req, res, next) => {
    ...
    if (user) {
        join.find(res, user, function (result) {
            res.render('joinmeeting', { total: result.length, meetingList: result });
        })
    }
    else {
        res.redirect('/');
...
router.get('/as-invitee', (req, res, next) => {
    ...
    if (user) {
        join.accptMeetingList(res, user, function (result) {
            res.render('asInvitee', { total: result.length, meetingList: result });
        })
    }
    else {
        res.redirect('/');
...
router.get('/as-admin', (req, res, next) => {
    ...
    let user = req.user;
    if (user) {
        join.accptMeetingListAsAdmin(res, user, function (result) {
            var filter = []
            if (result.length > 0) {
                for (let index = 0; index < result.length; index++) {
                    ...
                    if (element.userID != user.id) {
                        ...
                        filter.push({ ...element, date })
                    }
                }
            }
            res.render('asAdmin', { fullname: user.fullname, meetingList: filter });
        })
    }
    else {
        res.redirect('/');
...
router.post('/setmeeting', (req, res, next) => {
    ...
    if (user) {
        ...
        slot.create(res, userInput, function (lastId) {
            if (lastId) {
                res.send({ save: true, message: "Your Slot Data Has Been Saved" })
            } else {
                console.log('Error creating a new user ...');
                res.send("Something went Wrong")
            }
...
router.post('/acceptmeeting', (req, res, next) => {
    ...
    if (user) {
        ...
        slot.create(res, userInput, function (lastId) {
            if (lastId) {
                res.send({ save: true, message: "Your Slot For Meeting Has Been Saved" })
            } else {
                console.log('Error creating a new user ...');
                res.send("Something went Wrong")
            }
